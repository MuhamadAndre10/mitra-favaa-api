...
type UserAccounts struct {
	...
	Customer       Partner          `gorm:"foreignKey:UserID;references:ID"`
	CreatedAt      time.Time        `gorm:"column:created_at;not null;autoCreateTime"`
	UpdatedAt      time.Time        `gorm:"column:updated_at;not null;autoUpdateTime"`
	DeletedAt      time.Time        `gorm:"column:deleted_at;not null"`
	verifications  VerificationData `gorm:"foreignKey:Email;references:Email"`
}

// BeforeCreate a hook before insert into database
